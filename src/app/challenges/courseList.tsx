...
import { MountStatus, employeeOverview } from "../admin/dashboard/employeeDefinitions";
...
const SubmitModal = ({
  ...
  const handleSignIn = async () => {
    ...
    const allowed = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z", "1", "2", "3", "4", "5", "6", "7", "8", "9", "0", "-"]
    for (const i of username) {
      if (!(allowed.includes(i))) {
        setErrorMessage("Username must only contain lowercase letters, numbers, and hypens");
        setFieldErrors({ ...errors, username: true });
        return;
      }
    }
...
return (
  <div id="card-container" className="space-y-4">
    {_DATA.map((courseData: Course, courseIndex: number) => (
      <Card key={courseIndex} id={courseData.course + courseData.assignments.map((element) => element.name).join()} className="w-full p-0 shadow-md">
              ...
              >
                <div>
                  ...
                  <div className="mt-1 flex items-center space-x-2">
                    <span
                      className={`rounded-full px-2 py-1 text-sm font-semibold ${assignment.status === "done"
                        ? "bg-green text-white"
                ...
                <div className="flex flex-col items-end space-y-2">
                  <Button className="w-32 rounded-md bg-blue px-16 py-5 text-sm text-white hover:bg-slate-100" onClick={openModal}>
                    Link to Lesson
                  </Button>
                </div>
              </div>
