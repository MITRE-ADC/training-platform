...
import { assignments, courses, users, user_assignments, user_courses } from "@/db/schema";
...
export async function GET(request: NextRequest){
    // const search_strings = request.nextUrl.searchParams?.get("search_string")?.split(',') || []; // NOT SURE HOW TO IMPLEMENT---> ASK LATER
    const course_filter = request.nextUrl.searchParams?.get("course_filter")?.split(',') || [];
    const assignment_filter = request.nextUrl.searchParams?.get("assignment_filter")?.split(',') || [];
    ...
    if (course_filter==null || assignment_filter == null || status_filter == null){
  ...
  if (course_filter.length > 0) {
    ...
    const userCourses = await db
      ...
      .from(user_courses)
      .where(inArray(user_courses.course_id, filteredCourseIds))
      .execute();
  ...
  if (assignment_filter.length > 0) {
    ...
    const filteredUserIdsForAssignments = userAssignments.map((row) => row.user_id);
  ...
  if (query === undefined){
    ...
    return NextResponse.json({data: usersResult }, {status: HttpStatusCode.Ok});
  }
